...
void Project::UpdateInterface()
{
	...
	SystemUpdateCurrentCamera(NULL, m_pViewCameras[m_nActiveViewport], m_Cameras);
	UpdateSelection();
	if (m_bAnimation)
...
void Project::LoadDefaults(bool cameras)
{
	...
	if (cameras)
	{
		...
		SystemUpdateCameraMenu(m_Cameras);
		SystemUpdateCurrentCamera(NULL, m_pViewCameras[0], m_Cameras);
	}
	...
	{	
		if (fv > 0.4f)
		{
			...
			if (pInfo)
			{
				...
				if (bMerge)
					for (lcPiece* p = m_Pieces; p; p = (lcPiece*)p->m_Next)
		...
		else
		{
			...
			if (Info != NULL)
			{
				...
				Piece->Initialize(pos[0], pos[1], pos[2], step, 1, color);
				Piece->CreateName(m_Pieces);
				AddPiece(Piece);
				mat.CreateOld(0,0,0, rot[0],rot[1],rot[2]);
				mat.ToAxisAngle(param);
				Piece->ChangeKey(1, false, false, param, LC_PK_ROTATION);
				Piece->ChangeKey(1, true, false, param, LC_PK_ROTATION);
	...
	if (!bMerge)
	{
		if (fv >= 0.6f)
		{
			...
			if (count < 7)
			{
				...
				for (pCam = m_Cameras; pCam; pCam = (lcCamera*)pCam->m_Next)
					pCam->FileLoad(*file);
		}
		...
		if (fv >= 0.7f)
		{
			for (count = 0; count < 4; count++)
			{
				...
				lcCamera* pCam = m_Cameras;
				while (i--)
					pCam = (lcCamera*)pCam->m_Next;
...
void Project::FileSave(File* file, bool bUndo)
{
	...
	lcPiece* Piece;
	for (i = 0, Piece = m_Pieces; Piece; Piece = (lcPiece*)Piece->m_Next)
		i++;
	file->WriteLong(&i, 1);
...
bool Project::SetActiveView(View* view)
{
	...
	{
		view->Redraw();
		SystemUpdateCurrentCamera(OldCamera, m_ActiveView->GetCamera(), m_Cameras);
	}
...
void Project::RenderScene(bool bShaded, bool bDrawViewports)
{
	...
	for (int vp = 0; vp < viewports[m_nViewportMode].n; vp++)
	{
		...
		{
			Matrix44 Mats[3];
			Mats[0] = m_pViewCameras[vp]->m_WorldView;
		...
		if ((m_nDetail & LC_DET_LIGHTING) != 0)
		{
			...
			for (pLight = m_Lights; pLight; pLight = (lcLight*)pLight->m_Next, index++)
				pLight->Setup(index);
		}
...
*/
			...
			{
				BuildBSP(&tree, pList);
				RenderBSP(&tree, m_pViewCameras[vp]->m_Position, &bSel,
			...
			for (pPiece = m_Pieces; pPiece; pPiece = pPiece->m_Next)
			{
//				MSG msg;
...
void Project::RenderViewports(bool bBackground, bool bLines)
{
	...
	if (bLines)
	{
		...
		for (vp = 0; vp < viewports[m_nViewportMode].n; vp++)
		{
			...
			m_pScreenFont->PrintText(x + 3, y + h - 6, 0.0f, m_pViewCameras[vp]->m_Name);
		}
...
void Project::RemovePiece(lcPiece* pPiece)
{
	...
	for (pTemp = m_Pieces; pTemp; pLast = pTemp, pTemp = (lcPiece*)pTemp->m_Next)
		if (pTemp == pPiece)
		{
			if (pLast != NULL)
				pLast->m_Next = pTemp->m_Next;
			else
				m_Pieces = (lcPiece*)pTemp->m_Next;
...
//	CWaitCursor wc;
			...
			float up[3];
			const Vector3& eye = m_pViewCameras[m_nActiveViewport]->m_Position;
			const Vector3& target = m_pViewCameras[m_nActiveViewport]->m_TargetPosition;
			m_pViewCameras[m_nActiveViewport]->GetUpVec (up);

			fprintf(f, "\ncamera {\n  sky<%1g,%1g,%1g>\n  location <%1g, %1g, %1g>\n  look_at <%1g, %1g, %1g>\n  angle %.0f\n}\n\n",
				up[0], up[1], up[2], eye[1], eye[0], eye[2], target[1], target[0], target[2], m_pViewCameras[m_nActiveViewport]->m_FOV);
...
//			lcLight* pLight;
			...
			for (i = 0, pCamera = m_Cameras; pCamera; pCamera = (lcCamera*)pCamera->m_Next)
...
//	pFrame->UpdateInfo();
		...
		case LC_PIECE_INSERT:
		{
			...
			for (pLast = m_Pieces; pLast; pLast = (lcPiece*)pLast->m_Next)
			...
			if (pLast != NULL)
			{
				...
				pPiece->Initialize(Pos[0], Pos[1], Pos[2], m_nCurStep, m_nCurFrame, g_App->m_SelectedColor);
				...
				pPiece->Initialize(0, 0, 0, m_nCurStep, m_nCurFrame, g_App->m_SelectedColor);
...
//				SystemUpdateFocus(NULL);
		...
		case LC_PIECE_GROUP_EDIT:
		{
			...
			for (i = 0, pPiece = m_Pieces; pPiece; pPiece = (lcPiece*)pPiece->m_Next)
				i++;
			opts.piececount = i;
			opts.pieces = (lcPiece**)malloc(i*sizeof(lcPiece*));
			opts.piecesgroups = (Group**)malloc(i*sizeof(Group*));

			for (i = 0, pPiece = m_Pieces; pPiece; pPiece = (lcPiece*)pPiece->m_Next, i++)
			{
		...
		case LC_PIECE_PREVIOUS:
		{
			...
			for (lcPiece* Piece = m_Pieces; Piece; Piece = (lcPiece*)Piece->m_Next)
				if (Piece->IsSelected())
				{
					if (m_bAnimation)
					{
						unsigned short t = Piece->GetFrameShow();
						if (t > 1)
						{
							Redraw = true;
							Piece->SetFrameShow(t-1);
						}
					}
					else
					{
				...
				if (Piece->IsSelected())
				{
					if (m_bAnimation)
					{
						...
						if (t < m_nTotalFrames)
						{
							...
							if (Piece->IsSelected () && t == m_nCurFrame)
		...
		case LC_VIEW_ZOOM:
		{
			m_pViewCameras[m_nActiveViewport]->DoZoom(nParam, g_App->m_MouseSensitivity, m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation, m_bAddKeys);
			SystemUpdateFocus(NULL);
			UpdateOverlayScale();
			UpdateAllViews();
		} break;
		...
		case LC_VIEW_ZOOMIN:
		{
			m_pViewCameras[m_nActiveViewport]->DoZoom(-1, g_App->m_MouseSensitivity, m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation, m_bAddKeys);
			SystemUpdateFocus(NULL);
			UpdateOverlayScale();
			UpdateAllViews();
		} break;
		...
		{
			// FIXME: rewrite using the FrustumCull function
			if (m_Pieces == 0) break;
		...
		case LC_VIEW_VIEWPORTS:
		{
			...
			{
				SystemUpdateCurrentCamera(m_pViewCameras[m_nActiveViewport], m_pViewCameras[0], m_Cameras);
				m_nActiveViewport = 0;
			}
		...
		{
			for (lcPiece* Piece = m_Pieces; Piece; Piece = (lcPiece*)Piece->m_Next)
				Piece->InsertTime(m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation, 1);

			for (lcCamera* Camera = m_Cameras; Camera; Camera = (lcCamera*)Camera->m_Next)
				Camera->InsertTime(m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation, 1);

			for (lcLight* Light = m_Lights; Light; Light = (lcLight*)Light->m_Next)
				Light->InsertTime(m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation, 1);
		...
		{
			for (lcPiece* Piece = m_Pieces; Piece; Piece = (lcPiece*)Piece->m_Next)
				Piece->RemoveTime(m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation, 1);

			for (lcCamera* Camera = m_Cameras; Camera; Camera = (lcCamera*)Camera->m_Next)
				Camera->RemoveTime(m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation, 1);

			for (lcLight* Light = m_Lights; Light; Light = (lcLight*)Light->m_Next)
				Light->RemoveTime(m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation, 1);
		...
		case LC_VIEW_CAMERA_MENU:
		{
			...
			SystemUpdateCurrentCamera(m_pViewCameras[m_nActiveViewport], Camera, m_Cameras);
			m_pViewCameras[m_nActiveViewport] = Camera;
			UpdateOverlayScale();
			UpdateAllViews();
		} break;
		...
		case LC_VIEW_CAMERA_RESET:
		{
			...
			for (m_Cameras = pCamera = NULL, i = 0; i < 7; i++)
			{
...
lcCamera* Project::GetCamera(int i) const
{
	...
	for (pCamera = m_Cameras; i-- > 0 && pCamera; pCamera = (lcCamera*)pCamera->m_Next)
		;
	return pCamera;
}
...
void Project::GetActiveViewportMatrices(Matrix44& ModelView, Matrix44& Projection, int Viewport[4])
{
	...
	// Build the matrices.
	ModelView = Cam->m_WorldView;
	Projection = CreatePerspectiveMatrix(Cam->m_FOV, Aspect, Cam->m_NearDist, Cam->m_FarDist);
}
...
lcObject* Project::GetFocusObject() const
{
	...
	for (lcCamera* pCamera = m_Cameras; pCamera; pCamera = (lcCamera*)pCamera->m_Next)
	{
...
void Project::GetPieceInsertPosition(int MouseX, int MouseY, Vector3& Position, Vector4& Rotation)
{
	...
	// Build the matrices.
	Matrix44 Projection;
	const Matrix44& ModelView = Cam->m_WorldView;
	Projection = CreatePerspectiveMatrix(Cam->m_FOV, Aspect, Cam->m_NearDist, Cam->m_FarDist);
...
lcObject* Project::FindObjectFromPoint(int x, int y, bool PiecesOnly)
{
	...
	ClickLine.Start = Vector3(px, py, pz);
	ClickLine.End = Vector3(rx, ry, rz) - ClickLine.Start;
	...
	{
		for (lcCamera* Camera = m_Cameras; Camera; Camera = (lcCamera*)Camera->m_Next)
			if (Camera != m_pViewCameras[m_nActiveViewport])
...
void Project::FindObjectsInBox(float x1, float y1, float x2, float y2, lcPtrArray<lcObject>& Objects)
{
	...
	for (lcPiece* Piece = m_Pieces; Piece != NULL; Piece = (lcPiece*)Piece->m_Next)
	{
		if (!Piece->IsVisible(m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation))
			continue;
	...
	for (lcCamera* Camera = m_Cameras; Camera != NULL; Camera = (lcCamera*)Camera->m_Next)
	{
		if (!Camera->IsVisible())
			continue;
	...
	for (lcLight* Light = m_Lights; Light != NULL; Light = (lcLight*)Light->m_Next)
	{
		if (!Light->IsVisible())
			continue;
...
bool Project::SetActiveViewport(int px, int py)
{
	...
	for (vp = 0; vp < viewports[m_nViewportMode].n; vp++)
	{
		...
		if (px > x && px < x + w && py > y && py < y + h)
		{
			...
			{
				SystemUpdateCurrentCamera(m_pViewCameras[m_nActiveViewport], m_pViewCameras[vp], m_Cameras);
	...
	if (bAccept)
	{
		switch (m_nCurAction)
		{
			...
			case LC_ACTION_ZOOM_REGION:
			{
				...
				// Center camera.
				Vector3 Eye = Cam->m_Position;
				Eye = Eye + (Points[0] - Points[1]);

				Vector3 Target = Cam->m_TargetPosition;
...
bool Project::OnKeyDown(char nKey, bool bControl, bool bShift)
{
	...
	switch (nKey)
	{
		...
		case KEY_TAB:
		{
			...
			for (pFocus = m_Pieces; pFocus; pFocus = (lcPiece*)pFocus->m_Next)
				if (pFocus->IsFocused())
					break;
...
//						CalculateStep();
					...
					case LC_OBJECT_CAMERA_TARGET:
					{
						...
						{
							lcCamera* pPrev;
							for (pPrev = m_Cameras; pPrev; pPrev = (lcCamera*)pPrev->m_Next)
...
//				AfxGetMainWnd()->PostMessage(WM_LC_UPDATE_INFO, NULL, OT_PIECE);
		...
		case LC_ACTION_LIGHT:
		{
			...
			else if (m_nCurAction == LC_ACTION_LIGHT)
			{
				...
				pLight->CreateName(m_Lights);
				pLight->m_Next = m_Lights;
				m_Lights = pLight;
    ...
    case LC_ACTION_CAMERA:
    {
      ...
      SystemUpdateFocus(pCamera);
		...
		case LC_ACTION_MOVE:
		{
			...
			if (!sel)
			{
				for (lcLight* pLight = m_Lights; pLight; pLight = (lcLight*)pLight->m_Next)
				{
					if (pLight->IsSelected())
					{
						sel = true;
...
void Project::OnMouseMove(View* view, int x, int y, bool bControl, bool bShift)
{
	...
	switch (m_nCurAction)
	{
		...
		case LC_ACTION_ZOOM:
		{
			...
			m_pViewCameras[m_nActiveViewport]->DoZoom(y - m_nDownY, g_App->m_MouseSensitivity, m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation, m_bAddKeys);
			m_nDownY = y;
			SystemUpdateFocus(NULL);
			UpdateAllViews();
		} break;
		...
		case LC_ACTION_PAN:
		{
			...
			m_pViewCameras[m_nActiveViewport]->DoPan(x - m_nDownX, y - m_nDownY, g_App->m_MouseSensitivity, m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation, m_bAddKeys);
			m_nDownX = x;
			m_nDownY = y;
			SystemUpdateFocus(NULL);
			UpdateAllViews();
		...
		case LC_ACTION_ROTATE_VIEW:
		{
			...
			{
				case LC_OVERLAY_XYZ:
					m_pViewCameras[m_nActiveViewport]->DoRotate(x - m_nDownX, y - m_nDownY, g_App->m_MouseSensitivity, m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation, m_bAddKeys, bs);
					break;

				case LC_OVERLAY_X:
					m_pViewCameras[m_nActiveViewport]->DoRotate(x - m_nDownX, 0, g_App->m_MouseSensitivity, m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation, m_bAddKeys, bs);
					break;

				case LC_OVERLAY_Y:
					m_pViewCameras[m_nActiveViewport]->DoRotate(0, y - m_nDownY, g_App->m_MouseSensitivity, m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation, m_bAddKeys, bs);
					break;

				case LC_OVERLAY_Z:
					m_pViewCameras[m_nActiveViewport]->DoRoll(x - m_nDownX, g_App->m_MouseSensitivity, m_bAnimation ? m_nCurFrame : m_nCurStep, m_bAnimation, m_bAddKeys);
					break;
			}
